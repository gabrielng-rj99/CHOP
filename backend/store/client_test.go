...
func TestArchiveAndDeleteClientWithPermanentContract(t *testing.T) {
	...
	contractID, err := InsertTestContract(db, "Licença Permanente", "KEY-PERM", startDate, endDate, subcategoryID, clientID, nil)
	if err != nil {
...
func TestDeleteClientPermanentlyWithActiveContracts(t *testing.T) {
	...
	now := time.Now()
	_, err = InsertTestContract(db, "Expiring Contract", "KEY-EXP-001", timePtr(now.AddDate(0, 0, -10)), timePtr(now.AddDate(0, 0, 30)), subcategoryID, clientID, nil)
	if err != nil {
...
func TestDeleteClientPermanentlyWithExpiredContracts(t *testing.T) {
	...
	now := time.Now()
	_, err = InsertTestContract(db, "Expired Contract", "KEY-EXPIRED-001", timePtr(now.AddDate(0, 0, -30)), timePtr(now.AddDate(0, 0, -5)), subcategoryID, clientID, nil)
	if err != nil {
		t.Fatalf("Failed to insert test contract: %v", err)
	}
...
func TestGetAllClientsIncludingArchived(t *testing.T) {
	db := setupClientTest(t)
	defer CloseDB(db)
	if err := ClearTables(db); err != nil {
		t.Fatalf("Failed to clear tables: %v", err)
	...
	if err != nil {
		t.Fatalf("Failed to create active client: %v", err)
	}

	// Create one archived client
	reg2 := "11.222.333/0001-81"
	id2, err := clientStore.CreateClient(domain.Client{
		Name:           "Archived Client",
		RegistrationID: &reg2,
		Status:         "ativo",
	})
	if err != nil {
		t.Fatalf("Failed to create client to archive: %v", err)
	}
	err = clientStore.ArchiveClient(id2)
	if err != nil {
		t.Fatalf("Failed to archive client: %v", err)
	}

	// Verify GetAllClients (should return 1)
	activeClients, err := clientStore.GetAllClients()
	if err != nil {
	...
	// Verify GetAllClientsIncludingArchived (should return 2)
	allClients, err := clientStore.GetAllClientsIncludingArchived()
	if err != nil {
		t.Fatalf("GetAllClientsIncludingArchived failed: %v", err)
	}
	if len(allClients) != 2 {
		t.Errorf("Expected 2 total clients, got %d", len(allClients))
	}
	...
	for _, e := range allClients {
		if e.ID == id2 {
			foundArchived = true
			if e.ArchivedAt == nil {
				t.Error("Archived client should have ArchivedAt set")
			}
		}
	}
	if !foundArchived {
		t.Error("Archived client not found in all clients list")
	}
}
