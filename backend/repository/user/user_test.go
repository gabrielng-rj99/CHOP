...
func TestEditUserAdminFlagPermissions(t *testing.T) {
	...
	_, err = userStore.CreateUser("admin", "Administrador", "SenhaForteRoot123!@#abcd", "root")
	if err != nil {
		t.Fatalf("Erro ao criar usuário root: %v", err)
	}

	// Cria usuário admin0
	_, err = userStore.CreateUser("admin0", "Admin Zero", "SenhaForteAdmin123!@#", "admin")
	if err != nil {
...
func setupUserTest(t *testing.T) *sql.DB {
	...
	if err := repository.ClearTables(db); err != nil {
...
func TestListUsers(t *testing.T) {
	...
	users := []struct {
		...
		{"user1", "User One", "Pass1@#Secure123a", "user"},
		{"user2", "User Two", "Pass2@#Secure123b", "user"},
		{"admin1", "Admin One", "Pass3@#Secure123abcd", "admin"},
	}
