...
func NewAffiliateStore(db repository.DBInterface) *AffiliateStore {
...
func (s *AffiliateStore) CreateAffiliate(affiliate domain.Affiliate) (string, error) {
	...
	trimmedName, err := repository.ValidateName(affiliate.Name, 255)
	if err != nil {
		return "", err
	}
	...
	description := repository.NormalizeOptionalString(affiliate.Description)
	email := repository.NormalizeOptionalString(affiliate.Email)
	phone := repository.NormalizeOptionalString(affiliate.Phone)
	address := repository.NormalizeOptionalString(affiliate.Address)
	notes := repository.NormalizeOptionalString(affiliate.Notes)
	tags := repository.NormalizeOptionalString(affiliate.Tags)
	contactPreference := repository.NormalizeOptionalString(affiliate.ContactPreference)
	documents := repository.NormalizeOptionalString(affiliate.Documents)
